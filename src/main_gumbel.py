...
def save_loss_curve(grad_est, losses, problem_agent=None, actions_to_learn=None):
    ...
    plt.ylabel('Loss')
    #plt.grid(True)
    plt.savefig('loss_curves/loss_curve_{}_{}_{}.png'.format(grad_est, problem_agent, actions_to_learn))
...
def train_masks(agent: PPOAgent, trajectory: Trajectory, args: Args, problem_agent=None, actions_to_learn=None):
    ...
    mask_sampled = gumbel_softmax(mask, args.gumbel_t_init, hard=True)
    ...
    for step in range(args.mask_learning_steps):
        ...
        mask_sampled = gumbel_softmax(mask, args.gumbel_t, hard=False)
        ...
        if mask_loss < best_loss:
            best_loss = mask_loss
            best_mask = gumbel_softmax(mask, args.gumbel_t, hard=True)
    save_loss_curve(args.grad_est, mask_losses, problem_agent, actions_to_learn)
    args.gumbel_t = args.gumbel_t_init#restore t
