...
def gumbel_softmax(logits, temperature, hard=False):
    ...
    if hard:
        max_indices = torch.argmax(logits, dim=0, keepdim=True)
        one_hot = torch.zeros_like(logits)
        one_hot.scatter_(0, max_indices, 1.0)
        y = (one_hot - y).detach() + y
    return y

def save_loss_curve(args, losses, problem_agent=None, actions_to_learn=None):
    steps = list(range(len(losses)))
    steps = [20*i for i in steps]
    plt.figure()
    plt.plot(steps, losses)
    plt.title('Loss curve for {}'.format(args.grad_est))
    plt.xlabel('Step')
    plt.ylabel('Loss')
    #plt.grid(True)
    plt.savefig('loss_curves/loss_curve_{}_{}_{}_{}.png'.format(problem_agent, actions_to_learn, args.grad_est, args.seed))

def masking(args, mask_logits, temp=None, hard=True):
    if args.grad_est == "STE":
        mask_probs = torch.softmax(mask_logits, dim=0)
        mask_discretized = STESoftmax.apply(mask_probs)
        return mask_discretized
    elif args.grad_est == "Gumbel":
        mask_sampled = gumbel_softmax(mask_logits, temp, hard)
        return mask_sampled
...
def train_masks(agent: PPOAgent, trajectory: Trajectory, args: Args, problem_agent=None, actions_to_learn=None):
    ...
    #mask_discretized = STESoftmax.apply(mask_probs)
    mask_sampled = masking(args, mask, temp=args.gumbel_t_init, hard=True)
    ...
    init_trajectory = agent.run_with_mask(observations, mask_sampled, trajectory.get_length())
    init_loss = loss_fn(torch.stack(init_trajectory.logits), torch.tensor(actions))

    best_mask = mask_sampled
    best_loss = init_loss
    ...
    for step in range(args.mask_learning_steps):
        #mask_probs = torch.softmax(mask, dim=0)
        #mask_discretized = STESoftmax.apply(mask_probs)
        if args.grad_est == "Gumbel":
            if  step % 50 == 0:
                args.gumbel_t = max(np.exp((step+1) * -1 * args.gumbel_decay), 0.1)
        mask_sampled = masking(args, mask, temp=args.gumbel_t, hard=False)
        new_trajectory = agent.run_with_mask(observations, mask_sampled, trajectory.get_length())
        ...
        if mask_loss < best_loss:
            best_loss = mask_loss
            best_mask = masking(args, mask, temp=args.gumbel_t_init, hard=True)

    save_loss_curve(args, mask_losses, problem_agent, actions_to_learn)

    args.gumbel_t = args.gumbel_t_init#restored for next train
...
def evaluate_mask(agent: PPOAgent, mask: torch.nn.Parameter, target_sequence: List, test_problems: dict, length_sequence: int, args: Args):
    agent.eval()
    masked_score = 0
    original_score = 0
    total_comparisons = 0
...
def learn_masks(args: Args):
    ...
    for problem_agent, agent in problem_agent_dict.items():
        ...
        for actions_to_learn, trajectory in sequences_to_learn.items():
            ...
            masked_score, original_score = evaluate_mask(agent, trained_mask, actions_to_learn, test_problems, args.number_actions, args)
